...
public class NuSpecCreator {
    public async Task<XDocument> CreateNuSpecAsync(string projectFileFullName,
        ...
        var configuration = JsonSerializer.Deserialize<ConfigurationCopy>(await File.ReadAllTextAsync("settings.json"));
        ...
        var version = JsonSerializer.Deserialize<VersionCopy>(await File.ReadAllTextAsync(versionFile));
        if (version == null) {
            return document;
        }
    ...
    protected static XElement? ReadMetaData(string solutionId, string checkedOutBranch, XDocument projectDocument,
            ...
            VersionCopy version, string targetFramework, string organizationUrl, string author, string faviconUrl,
    ...
    protected static XElement? Files(XDocument projectDocument, XmlNamespaceManager namespaceManager, XNamespace nugetNamespace, ConfigurationCopy configuration) {
        var rootNamespaceElement = projectDocument.XPathSelectElements("./Project/PropertyGroup/RootNamespace", namespaceManager).FirstOrDefault();
    ...
    private static string? IncludeAttributeValue(XElement contentElement, ConfigurationCopy configuration) {
    ...
    private static bool IncludesFileToPack(XElement contentElement, ConfigurationCopy configuration) {
